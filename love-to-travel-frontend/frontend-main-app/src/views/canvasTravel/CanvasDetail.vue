...
import {
  getCanvasOneSceneryInfoByName,
  getCanvasOneCityInfoByName,
...
const getCanvasDataAndInitNode = async () => {
  await getCanvasOneCityInfoByName(cityName)
    .then((res: any) => {
      if (res.code != 0) {
        //@ts-ignore
        ElMessage({
...
const getCanvasDataAndInitNodeLink = async () => {
  await getCanvasOneCityInfoByName(cityName)
    .then((res: any) => {
      if (res.code != 0) {
        //@ts-ignore
...
const requestNodeRelByName = async ($event, d) => {
  ...
  await getCanvasOneSceneryInfoByName(d.id)
    .then((res: any) => {
      if (res.code != 0) {
        //@ts-ignore
        ElMessage({
...
const useQueryResult = (result, currentNode, currentType) => {
  ...
  for (let j = 0; j < canvasData.nodes3.length; j++) {
    will_del_links.forEach((e) => {
      ...
      new_links.forEach((e2) => {
        //想删除但是现有连线用到ta了
        if (
          // @ts-ignore
          canvasData.nodes3[j].id === e &&
          // @ts-ignore
          canvasData.nodes3[j].id === e2.target
        ) {
          new_nodes.push(canvasData.nodes3[j]);
        }
      });
  ...
  links = g
    ...
    .attr(
      ...
      (d) =>
        ...
        // } else {
        //   // @ts-ignore
        d.source + "_" + d.relationship + "_" + d.target
      // }
    ); //一对关系的id是唯一的
...
const initGraph2 = (data) => {
  ...
  linksName = g
    ...
    .text(function (d) {
      // @ts-ignore
      //   console.log(typeof d.id); //string
      //   let relationship = d.id.match(/_(\S*)_/)[1];
      //   console.log(relationship);
      // @ts-ignore
      //   alert(d.relationship);
      //   alert(d.target);
      // @ts-ignore
...
<template>
  ...
  <detail-panel
    ...
    @changeModel="changeTheLinkNodeModel"
  ></detail-panel>
  <!-- </div> -->
</template>
