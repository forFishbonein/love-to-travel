...
footData = pd.read_csv('data/foot.csv', dtype='str')
# footData=DataProcess.footInfor()
# print(footData.shape)

iterm = footData['景区码'].drop_duplicates().reset_index()
iterm_size = len(iterm)
user = footData['用户码'].drop_duplicates().reset_index()
user_size = len(user)

consume_times = footData[{'用户码','景区码'}]
...
DataProcess.SceneryData()
# pd.DataFrame.to_csv(iterm,r"D:\暂存文件\recommenderSystemForFlowerShop-master\recommenderSystemForFlowerShop-master\scenery_test.csv",columns=['景区码','itermId'],encoding="GBK")
index2ItermDict=dict( zip(iterm['景区码'],iterm['景区码']))
iterm2IndexDict=dict( zip(iterm['景区码'],iterm['景区码']))
# 保存物品编号ID和物品码的对应关系，方便以后查找具体物品。
...
# pd.DataFrame.to_csv(user,r"D:\暂存文件\recommenderSystemForFlowerShop-master\recommenderSystemForFlowerShop-master\User_test.csv",columns=['用户码','userId'],encoding="GBK")
user2IndexDict=dict(zip(user['用户码'],user['用户码']))
index2UserDict=dict(zip(user['用户码'],user['用户码']))

# 增加一列计数列，从1开始
consume_times['times']=int(1)
# 把产品码和用户码替换成id，便于构造相关矩阵。
consume_times['景区码']=[iterm2IndexDict[x] for x in consume_times['景区码']]
...
secoreMatrixDatafreme = pd.DataFrame(consume_times.groupby(by=["用户码","景区码"]).agg('count'))
# 变换矩阵的索引列
x=secoreMatrixDatafreme.index.to_frame()
x.rename(columns={'用户码':'u','景区码':'i'},inplace = True)
# 把统计到的购买次数和产品码用户码拼接起来
# TODO 这里可以根据实际业务进行评分矩阵打分的构造，比如加入购买总数量和购买金额，注意考虑个维度的取值范围，也可以先进行归一化各个特征，然后按权重加总
scoreFeture= x.merge(secoreMatrixDatafreme, on=['用户码', '景区码'])
...
vecWord.vecUser_train()
# pd.DataFrame(U).to_csv("EmbedingUser.csv")
VT=np.array(V).transpose()
VT=pd.DataFrame(VT)

vecWord.vecSCenery_train()
# pd.DataFrame(VT).to_csv("data/EmbedingScenery.csv")
